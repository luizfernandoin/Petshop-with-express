...
petRouter.delete("/:id", checkExistsPetshop, async (request: Request, response: Response, next: NextFunction) => {
    const { id } = request.params;

    if (!request.petshop) {
        throw new HttpError('Petshop não está na requisição', 404);
    }
...
petRouter.put("/:id", checkExistsPetshop, async (request: Request, response: Response, next: NextFunction) => {
    const { id } = request.params;
